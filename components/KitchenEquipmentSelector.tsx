...
interface EquipmentItem {
  ...
  label: string;
  src: string;
  // Position as % of kitchen width/height; width as % of kitchen width
  left: number;
  top: number;
  width: number;
  zIndex: number;
...
const KitchenEquipmentSelector: React.FC<KitchenEquipmentSelectorProps> = ({
  ...
  return (
    <div className="w-full overflow-visible">
      ...
      {/* Kitchen scene (positions unchanged) */}
          ...
          {equipmentItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {selected && (
                  <span className="absolute -right-1 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-orange-200 text-[8px] font-black text-white shadow-lg sm:-right-2 sm:-top-2 sm:h-6 sm:w-6 sm:text-xs">
                    ✓
                  </span>
                )}
              </button>
      ...
      {/* List: easier tapping on small screens, same selection state */}
      <div className="mt-10 sm:mt-12">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            type="button"
            onClick={() =>
              onChange(allSelected ? [] : equipmentItems.map((i) => i.name))
            }
            className="text-sm text-stone-500 transition-colors hover:text-orange-200 focus:outline-none focus-visible:underline"
          >
            ...
            return (
              <li key={item.id} className="min-w-0">
                <button
                  ...
                  aria-pressed={selected}
                  onClick={() => toggleEquipment(item.name)}
                  className={`relative flex h-full w-full items-center gap-2 rounded-xl border px-2.5 py-2.5 text-left text-[13px] font-medium leading-tight transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-300/70 sm:gap-2.5 sm:px-3 sm:text-sm ${
                    selected
                      ? 'border-orange-400 bg-orange-400/10 text-stone-100'
